...
class SymmetryGroupTestCase( unittest.TestCase ):
    ...
    def test_get_numeric_equivalents_returns_correct_values(self):
        """Test that get_numeric_equivalents returns correct byte representations."""
        s0 = SymmetryOperation.from_vector([1, 2, 3])
        s1 = SymmetryOperation.from_vector([2, 1, 3])
        sg = SymmetryGroup(symmetry_operations=[s0, s1])
        
        config = Configuration([1, 0, 0])
        byte_equivalents = sg.get_numeric_equivalents(config)
    ...
    def test_get_numeric_equivalents_uses_unique_operations(self):
        """Test that get_numeric_equivalents only applies unique operations."""
        s0 = SymmetryOperation.from_vector([1, 2, 3])
        s1 = SymmetryOperation.from_vector([2, 1, 3])
        s2 = SymmetryOperation.from_vector([1, 2, 3])  # Duplicate of s0
        sg = SymmetryGroup(symmetry_operations=[s0, s1, s2])
    ...
    def test_get_numeric_equivalents_returns_set(self):
        """Test that get_numeric_equivalents returns a set."""
        s0 = SymmetryOperation.from_vector([1, 2, 3])
        sg = SymmetryGroup(symmetry_operations=[s0])
        
        config = Configuration([1, 0, 0])
        result = sg.get_numeric_equivalents(config)
        
        self.assertIsInstance(result, set)
    
    def test_get_numeric_equivalents_with_larger_configuration(self):
        """Test get_numeric_equivalents with larger configuration space."""
    ...
    def test_get_numeric_equivalents_returns_bytes(self):
        """Test that get_numeric_equivalents returns a set of bytes."""
        s0 = SymmetryOperation.from_vector([1, 2, 3])
        sg = SymmetryGroup(symmetry_operations=[s0])
    ...
    def test_get_numeric_equivalents_produces_different_values_for_different_configs(self):
        ...
        bytes1 = sg.get_numeric_equivalents(config1)
        bytes2 = sg.get_numeric_equivalents(config2)
