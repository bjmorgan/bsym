...
class TestConfiguration( unittest.TestCase ):
    ...
    def setUp( self ):
        self.configuration = Configuration( column_vector( [ 1, 0, 0 ] ) )
    ...
    def test_is_equivalent_to_if_not_equivalent( self ):
        test_configuration = Configuration( column_vector( [ 0, 1, 0 ] ) )
        symmetry_operations = [ Mock( spec=SymmetryOperation ) ]
        symmetry_operations[0].operate_on = Mock( return_value=Configuration( column_vector( [ 0, 0, 1 ] ) ) )
        self.assertEqual( self.configuration.is_equivalent_to( test_configuration, symmetry_operations ), False )

    def test_is_in_list( self ):
        configuration_list = [ Configuration( column_vector( [ 0, 1, 0 ] ) ), 
                               Configuration( column_vector( [ 1, 0, 0 ] ) ) ]
